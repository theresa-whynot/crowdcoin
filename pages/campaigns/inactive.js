...
class CampaignClosed extends Component {
    ...
    renderCampaigns() {
        ...
        const inactiveCampaigns = this.state.campaignDetails.filter(({ targetDeadline, sumContribution, minimumBalance }) => {
            const currentTimestamp = Math.floor(Date.now() / 1000);
            return (currentTimestamp > targetDeadline && sumContribution < minimumBalance)
        });

            const items = inactiveCampaigns.map(({ address, campaignTitle, campaignDescription }) => {
            return {
    ...
    render() {
        ...
        <div>
            ...
            <Link route="/campaigns/new">
                <a>
                    <Button floated="right" content="Create Campaign" icon="add circle" color='green' />
                </a>
            </Link>
